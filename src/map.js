  ...
  this.markers = this.svg
    ...
    .attr("transform", function(post) {
      return (
        ...
        self.projection(post.geo.geo)[0] +
        "," +
        self.projection(post.geo.geo)[1] +
        ")scale(" +
        MARKER_S_MIN +
        "," +
        MARKER_S_MIN +
      ...
      .html(function(post) {
        ...
        if (post.social[0].type == "twitter") {
          ...
          twtEmbed.splice(
            2,
            0,
            "<img class='rounded-circle' src='" + twtPofileURL + "'>"
          );
      ...
      Array.from(this.querySelectorAll("[data-social=chapter]")).forEach(
        function(chapter) {
          ...
          Array.from(chapter.querySelectorAll(".post")).forEach(function(
            ...
            if (
              self.isElementOnScreen(box, visibleEle, h / 2) &&
              ...
              markerOn
                ...
                .on("end", function() {
                  ...
                  self.toggleToolTip(
                    true,
                    [markerBox.x, markerBox.y],
                    markerOn.attr("data-loc")
                  );
                });
              ...
              if (lastPostId !== -1) {
                replaceClass(
                  document.getElementById(lastPostId),
                  "active",
                  "hidden"
                );
                ...
                markerOff
                  .attr(
                    ...
                    setTransform(
                      "translate",
                      getTransform(transformOff, "translate")
                    ) + setTransform("scale", [MARKER_S_MAX, MARKER_S_MAX])
                  )
                  .transition()
                  .ease(d3.easeExp)
